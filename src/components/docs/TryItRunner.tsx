...
const DEFAULT_GET_QUERY_PARAMS: QueryParamDef[] = [
  { name: 'q', type: 'string', description: 'Full-text search query term across name, username, email, title, etc.' },
...
export function TryItRunner({ endpoint }: TryItRunnerProps) {
  ...
  // Extract path parameters (e.g. :id, :userId, :postId)
  ...
  const computedUrl = useMemo(() => {
    ...
    pathParamsList.forEach((param) => {
      const val = pathValues[param] || `:${param}`;
  ...
  // Collapsed View (Matches Screenshot 1)
  if (!isOpen) {
    return (
      <div className="mt-4">
        <div
  ...
  // Expanded View (Matches Screenshot 2)
  return (
          ...
          {pathParamsList.map((param) => (
            <div key={param} className="space-y-1">
              <label className="text-xs font-semibold text-text-secondary flex items-center gap-1">
                <span>{param}</span>
                <span className="text-text-muted font-normal">(path):</span>
              </label>
              <input
                type="text"
                value={pathValues[param] || ''}
                onChange={(e) => setPathValues((prev) => ({ ...prev, [param]: e.target.value }))}
                placeholder={`User ID to update (e.g. 1 or local-<uuid>).`}
